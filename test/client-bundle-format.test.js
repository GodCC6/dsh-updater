...
test('locale DICT exposes uptodate in both languages with matching key sets', () => {
  ...
  // Fix round acceptance #3:短导航标签 + 点击即时反馈文案,双语逐字钉死
  assert.equal(dict.zh.nav, '更新')
  assert.equal(dict.en.nav, 'Updates')
  assert.equal(dict.zh.checking, '检查中…')
  assert.equal(dict.en.checking, 'Checking…')
  assert.equal(dict.zh.starting, '启动中…')
  assert.equal(dict.en.starting, 'Starting…')
